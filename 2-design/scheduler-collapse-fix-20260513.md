...
的任务；若队列中已经没有 immediate 任务，调用方必须显式传入 `allow_future=True`
才允许释放未来槽位任务补位。

2026-05-17 追加收紧：`BackgroundTaskRunner` 在刚完成全局 replan 或增量 merge 的同一轮，
使用 `allow_future=False`，只派发 immediate work item；只有后续非重规划轮次中，服务器
已有 slot 释放且计划未刷新时，才使用 `allow_future=True` 从既有 plan 中释放未来槽位任务。
这避免了刚生成的未来短任务抢占当前空闲槽，同时保留多槽服务器“完成一个补一个”的能力。

**文件**：`scheduler.py` `PlanPool.pop_dispatchable()`

### Fix 10：零并发节点不参与调度（P2）

当服务器 `status=ONLINE` 但 `max_concurrency=0` 时，旧逻辑仍可能进入调度候选，
导致配额、虚拟槽和补位逻辑出现不可执行计划。

修复：`schedule_batch()` 和 `BackgroundTaskRunner._dispatch_queued_tasks()` 均跳过
`max_concurrency <= 0` 的服务器，并输出 `server_skipped_invalid_concurrency` 日志，
等待 benchmark 或人工更新推荐并发后再参与调度。

**文件**：`scheduler.py` `schedule_batch()`；`task_runner.py` `_dispatch_queued_tasks()`

### Fix 11：多槽 work stealing 收益使用计划完成时间（P2）

多并发源服务器的 PlanPool 队列中，不同 `slot_index` 的任务是并行计划。旧逻辑把候选前方
所有任务时长串行相加，可能高估或低估源队列剩余时间，影响偷取判断。

修复：优先使用候选 `decision.estimated_finish` 作为源服务器保留该任务时的计划完成时间；
缺失时才回退到 duration 累加。这样 work stealing 的收益和槽位预规划保持同一时间口径。

**文件**：`task_runner.py` `_find_steal_candidate()`

## 6. 测试验证

- `test_scheduler.py`：82 passed（含 `test_online_server_with_invalid_zero_capacity_is_not_schedulable`、`test_pop_dispatchable_returns_empty_when_all_items_are_future`、`test_pop_dispatchable_can_release_future_items_after_server_is_idle`）
- `test_task_runner_dispatch.py`：47 passed（含 `test_partial_slot_completion_pops_future_item_from_existing_plan`、`test_find_steal_candidate_uses_parallel_finish_over_sequential_sum`）
- 调度相关合并验证：`test_scheduler.py + test_task_runner_dispatch.py` 共 129 passed（2026-05-17）
