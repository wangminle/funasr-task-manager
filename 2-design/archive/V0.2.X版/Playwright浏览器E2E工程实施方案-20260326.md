...
**Architecture:** 以前端目录作为 Playwright 宿主，使用 Playwright 配置同时拉起后端和前端开发服务，测试通过 fixture batch 选择小文件样本，运行后把截图和摘要写入 `7-data/outputs/e2e/<timestamp>/`。生产代码最小变更仅限于为上传页和任务列表页添加稳定的 `data-testid`。
...
把 run-summary 和截图写入 `7-data/outputs/e2e/<timestamp>/`。
...
3. 归档 `7-data/outputs/e2e/`，至少保留 `run-summary.json`、`run-summary.md`、`results/`、`screenshots/`
