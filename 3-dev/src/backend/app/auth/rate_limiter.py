...
class RateLimiter:
    ...
    def check_concurrent_tasks(self, user_id: str) -> None:
        if not self._enabled:
            return
        state = self._get_state(user_id)
        if state.concurrent_tasks >= self.config.max_concurrent_tasks:
            raise HTTPException(
                status_code=429,
                detail=f"Concurrent task limit reached ({self.config.max_concurrent_tasks})",
            )

    def check_daily_limit(self, user_id: str) -> None:
        if not self._enabled:
            return
        state = self._get_state(user_id)
        if state.daily_task_count >= self.config.max_tasks_per_day:
            raise HTTPException(
                status_code=429,
                detail=f"Daily task limit reached ({self.config.max_tasks_per_day})",
            )

    def check_upload_bandwidth(self, user_id: str, file_size: int) -> None:
        if not self._enabled:
            return
        state = self._get_state(user_id)
        now = time.time()
    ...
    def record_task_created(self, user_id: str) -> None:
        if not self._enabled:
            return
        state = self._get_state(user_id)
    ...
    def record_task_completed(self, user_id: str) -> None:
        if not self._enabled:
            return
        state = self._get_state(user_id)
