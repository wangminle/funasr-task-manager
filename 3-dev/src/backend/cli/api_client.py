...
class ASRClient:
    ...
    def _check(self, resp: httpx.Response) -> httpx.Response:
        if resp.status_code >= 400:
            ...
            if not detail:
                detail = (
                    f"服务返回 HTTP {resp.status_code} 但没有错误详情；"
                    f"请检查 CLI server 配置是否正确: python -m cli config get server"
                )
    ...
    def _request(self, method: str, url: str, **kwargs) -> httpx.Response:
        try:
            ...
            raise APIError(
                0,
                f"无法连接到服务器 {self._client.base_url}；"
                f"请检查后端是否启动、地址是否正确: python -m cli config get server",
            ) from exc
        except httpx.TimeoutException as exc:
            raise APIError(0, f"连接服务器 {self._client.base_url} 超时") from exc
        except httpx.HTTPError as exc:
            raise APIError(0, f"请求服务器 {self._client.base_url} 失败: {exc}") from exc
    ...
    def metrics(self) -> str:
        return self._check(self._request("get", "/metrics")).text

    def stats(self, global_stats: bool = False) -> dict:
        params = {}
        if global_stats:
            params["global"] = "true"
        return self._check(self._request("get", "/api/v1/stats", params=params)).json()
    ...
    def file_info(self, file_id: str) -> dict:
        return self._check(self._request("get", f"/api/v1/files/{file_id}")).json()
    ...
    def list_tasks(
        ...
        if group:
            params["group"] = group
        return self._check(self._request("get", "/api/v1/tasks", params=params)).json()

    def get_task(self, task_id: str) -> dict:
        return self._check(self._request("get", f"/api/v1/tasks/{task_id}")).json()

    def cancel_task(self, task_id: str) -> dict:
        return self._check(self._request("post", f"/api/v1/tasks/{task_id}/cancel")).json()

    def get_result(self, task_id: str, fmt: str = "json") -> str:
        resp = self._check(self._request("get", f"/api/v1/tasks/{task_id}/result", params={"format": fmt}))
        return resp.text
    ...
    def get_task_group(self, group_id: str) -> dict:
        return self._check(self._request("get", f"/api/v1/task-groups/{group_id}")).json()

    def list_group_tasks(self, group_id: str, page: int = 1, page_size: int = 100) -> dict:
        params = {"page": page, "page_size": page_size}
        return self._check(self._request("get", f"/api/v1/task-groups/{group_id}/tasks", params=params)).json()

    def get_group_results(self, group_id: str, fmt: str = "txt") -> str:
        resp = self._check(self._request("get", f"/api/v1/task-groups/{group_id}/results", params={"format": fmt}))
        return resp.text

    def delete_task_group(self, group_id: str) -> dict:
        return self._check(self._request("delete", f"/api/v1/task-groups/{group_id}")).json()
    ...
    def register_server(self, data: dict) -> dict:
        if not data.get("run_benchmark"):
            return self._check(self._request("post",
    ...
    def update_server(self, server_id: str, data: dict) -> dict:
        return self._check(self._request("patch", f"/api/v1/servers/{server_id}", json=data)).json()

    def delete_server(self, server_id: str) -> None:
        self._check(self._request("delete", f"/api/v1/servers/{server_id}"))
