...
V0.4.26 继承 2026-05-16 同一组 45 个本地音视频文件连续两轮批量转写验证结论，并在 2026-05-17 补充调度边界修复：
...
两轮输出目录分别为 `runtime/agent-local-batch/outputs/local-20260516/` 和 `runtime/agent-local-batch/outputs/local-20260516-v2/`，均生成 45 个结果文件，文件名和内容哈希一致。V0.4.26 额外修复 work-steal 多槽收益计算、虚拟 slot future 任务派发边界和零并发服务器防御。该结论表示本地 45 文件批量转写主路径已较稳定；服务下线恢复、故障注入、更多超长文件混合批次仍需单独验证。
