...
- **浏览器 E2E**：Playwright 工程已固定落地在前端目录，支持 `smoke`、`remote-standard`、`standard`、`full` 四档批次
...
npm install
npm run dev    # 默认监听 http://localhost:3000 (vite.config.js)
```

> **端口说明**：`npm run dev` 使用 `vite.config.js` 中配置的 3000 端口；一键启停脚本（`start.ps1`/`start.sh`）通过 `npx vite --port 5173` 强制使用 5173 端口。两种方式均通过 Vite proxy 将 `/api` 请求转发到后端 8000 端口。
...
- 前端界面：<http://localhost:5173>（一键脚本）或 <http://localhost:3000>（`npm run dev`）
- Prometheus：<http://localhost:9090>
- Alertmanager：<http://localhost:9093>
- Grafana：<http://localhost:3001>（默认账号 admin/admin）

> **注意**：当前 Docker Compose 仅包含后端 API、Redis 和监控组件，不含前端/Nginx 服务。前端需独立通过 `npm run dev` 或一键脚本启动。
...
│   │   ├── config/        # 配置文件
│   │   └── docker-compose.yaml
│   └── frontend/          # Vue 3 前端 + Playwright 浏览器 E2E
│       ├── playwright.config.js
│       ├── scripts/       # E2E 辅助脚本（跨平台 Python / 后端启动）
│       └── tests/e2e/     # 浏览器端到端测试
├── 4-tests/
│   ├── scripts/           # 测试脚本
│   │   ├── unit/          # 单元测试 (184 tests)
...
# 浏览器 E2E remote-standard（远端 FunASR 推荐标准批次）
npm run test:e2e:remote-standard

# 浏览器 E2E standard（较重本地批次，可能包含 100MB+ 素材）
npm run test:e2e:standard

# 浏览器 E2E full（发布前或大改后全量回归）
...
浏览器 E2E 的正式工程固定在 `3-dev/src/frontend/` 下。`7-data/outputs/e2e/`、`3-dev/src/frontend/test-results/`、`3-dev/src/frontend/playwright-report/` 属于本地测试工件，默认不提交 Git。
...
- [ASR 任务中转适配层调研报告 & 设计方案](2-design/archive/ASR任务中转适配层调研报告&设计方案-20260226.md)
- [项目实施方案（测试驱动验收）](2-design/archive/项目实施方案-测试驱动验收-20260227.md)
- [架构与技术选型调研](1-discussion/2026-02-26-ASR任务管理器-架构与技术选型调研报告.md)
- [端到端浏览器测试 Skill 设计方案](2-design/端到端浏览器测试Skill设计方案-20260319.md)
- [Playwright 浏览器 E2E 工程实施方案](2-design/Playwright浏览器E2E工程实施方案-20260326.md)
- [端到端测试现状、Playwright安装结论与剩余问题](2-design/端到端测试现状、Playwright安装结论与剩余问题-20260326.md)
