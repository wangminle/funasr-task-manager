...
```
├── 转写 1 个文件            → funasr-cli transcribe audio.mp4
├── 转写多个文件（自动并行）  → funasr-cli transcribe *.wav --format txt
├── 只提交不等待              → funasr-cli transcribe files --no-wait
├── 查看批次进度              → funasr-cli task list --group <group_id>
├── 下载批次结果              → funasr-cli task result --group <group_id> --format txt,srt
├── 管理 ASR 服务器           → funasr-cli server list / probe / benchmark
├── 排查系统问题              → funasr-cli doctor
...
| 任务调度 | 进程内 BackgroundTaskRunner（asyncio）/ Dramatiq + Redis（可选） |
