...
ASR 任务管理器支持注册多台 FunASR 服务器，使用 LPT（最长处理时间优先）+ 最早完工时间做首轮预规划，并按槽位队列持续补位；当快节点先清空自己的队列时，会从其他节点队列尾部工作窃取更适合的任务，以缩短整批完工时间。分段任务会遵守单个父任务的段级并发上限，某个 segment 暂时不可窃取时，调度器会跳过该候选继续寻找其它可窃取任务。
...
| **工作窃取** | 快节点空闲且自身队列清空时，可从其他节点队列尾部窃取更快完成的任务；若最佳候选 segment 已达到父任务并发上限，会在本轮跳过该候选继续寻找其它候选 |
