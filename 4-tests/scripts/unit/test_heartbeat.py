...
from app.services.probe import ServerCapabilities
...
class TestHeartbeatService:
    async def test_online_server_stays_online(self):
        ...
        with pytest.MonkeyPatch.context() as m:
            m.setattr("app.services.heartbeat.ServerProbe.probe", AsyncMock(return_value=caps))
            update_fn = AsyncMock()
    ...
    async def test_unreachable_with_expired_heartbeat_goes_offline(self):
        ...
        svc = HeartbeatService(interval=1, timeout=60)
        caps = ServerCapabilities(server_id="s1", host="10.0.0.1", port=10095, is_reachable=False, error="refused")
    ...
    async def test_unreachable_within_timeout_goes_degraded(self):
        ...
        svc = HeartbeatService(interval=1, timeout=60)
        caps = ServerCapabilities(server_id="s1", host="10.0.0.1", port=10095, is_reachable=False, error="timeout")
