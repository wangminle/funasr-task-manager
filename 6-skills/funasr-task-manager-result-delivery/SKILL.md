...
**通知跟踪记录**：与 `channel-intake` 一致，每条 `send_user_notice()` 必须记录 `notice_log` 条目（`seq`、`phase`、`adapter`、`delivered`、`message_id`）。最终报告中附上通知摘要。
...
| **通知不可用不标注** | 预检发现两个通道都不可用，但最终报告不提及 | 必须在结果交付消息追加"⚠ 实时通知不可用" |
...
| `result_filename` | `{original_filename_stem}_result.txt` |
...
> **统一规则**：结果文件名一律使用 `{stem}_result.{fmt}` 格式，与代码实现（`task-group download` 输出）保持一致。

| 场景 | 命名规则 | 示例 |
|------|---------|------|
| 单文件 txt | `{stem}_result.txt` | `会议录音.mp4` → `会议录音_result.txt` |
| 批量同名 | `{stem}_result_{序号}.txt` | 两个 `录音.wav` → `录音_result.txt`、`录音_result_1.txt` |
| SRT 格式 | `{stem}_result.srt` | `采访.mp3` → `采访_result.srt` |
| JSON 格式 | `{stem}_result.json` | `演讲.wav` → `演讲_result.json` |
...
| **通知预检全部失败** | 在结果交付消息尾部追加"⚠ 实时通知不可用：本次进度通知均未实时送达" | 静默跳过不标注 |
