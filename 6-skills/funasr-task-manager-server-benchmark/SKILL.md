...
解析 NDJSON 流式事件，定期向用户/调用者汇报进度。

事件详情见 `references/ndjson-events.md`，以下是汇报时机：

- `benchmark_start` → 通知"开始 benchmark，共 2 阶段"
- `phase_start` → "Phase {phase}: {description}..."
- `phase_progress` → "第 {rep}/{total_reps} 次采样: RTF = {rtf}"（静默，除非用户要求详细日志）
- `phase_complete` → "Phase 1 完成: 单线程 RTF = {single_rtf}"
- `gradient_start` → "并发梯度 N={concurrency}（{level_index}/{total_levels}）"
- `gradient_complete` → "N={concurrency}: throughput_rtf = {throughput_rtf}, wall={wall_clock_sec}s"
- `gradient_error` → "N={concurrency}: {error}"
- `benchmark_complete` → 服务层完成（Agent 可忽略，等待 API 终结事件）
- `benchmark_result` → 单节点最终结果（含完整 `ServerBenchmarkItem`）
- `all_complete` → 全量最终聚合结果（含 `results[]` 和 `capacity_comparison[]`）
