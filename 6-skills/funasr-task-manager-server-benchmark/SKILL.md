...
Benchmark 必须使用同一个、已验证的项目 CLI 入口完成前置检查、执行和结果核对，避免系统 Python 与项目 venv 混用导致 httpx/依赖行为不一致。

1. 在仓库根目录解析 CLI Python：
   - 优先候选：`3-dev/src/backend/.venv/Scripts/python.exe`
   - 其次候选：`.venv/Scripts/python.exe`
   - 最后 fallback：当前 shell 的 `python`
2. 对候选入口执行：

```bash
<PY> -c "import sys,httpx; print(sys.executable); print(sys.version.split()[0]); print(httpx.__version__)"
cd 3-dev/src/backend && <PY> -m cli --version
```

3. 只有 `python` 可执行、`httpx` 可 import、`cli --version` 返回 `asr-cli 0.4.28` 时，才允许继续 benchmark。
4. 后续所有 benchmark 相关命令必须复用同一个 `<PY>`；不得混用系统 Python 和 venv Python。
5. 若 `.venv` 入口存在但指向缺失解释器，应报告"项目 venv 损坏或迁移后未重建"，改用通过自检的 fallback，并在最终报告中标注。

## 执行流程

### Phase 1：前置检查

按上述安全约束依次执行 CHECK 1-4。任一检查不通过时，按场景决定放弃或等待确认。
...
```bash
python -m cli notify send --text "Phase 1: 单线程性能测量..." --chat-id "<chat_id>"
```
...
| Benchmark 已在运行（HTTP 409） | 报告"该节点已有 benchmark 正在运行，等待当前校准结束后重试" | 立即强制重试 |
| NDJSON 解析失败 | 记录原始行，报告"收到无法解析的进度事件" | 静默忽略 |
| 退化检测触发 | 解释退化原因和建议 | 忽略退化继续测试更高并发 |
| 队列非空但用户坚持 | 用户确认后继续，但在报告中标注"非空闲条件下执行" | 不警告直接执行 |
| 网络错误 / 连接断开 | 报告"与服务器通信中断"，先确认后端 health 和是否已有 benchmark 锁，再决定是否重试 | 自动连续重试 |
